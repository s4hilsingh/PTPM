...
function RecentPosts()
{
	...
	$request = $smcFunc['db_query']('', '
		SELECT
			m.id_msg, m.subject, m.smileys_enabled, m.poster_time, m.body, m.id_topic, t.id_board, b.id_cat, t.id_prefix,
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		$context['topics'][$row['id_topic']] = array(
			...
			'first_post' => array(
				...
				'link' => PostPrefix::formatPrefix($row['id_prefix']). ' <a href="' . $scripturl . '?topic=' . $row['id_topic'] . '.0;topicseen">' . $row['first_subject'] . '</a>'
			),
			'last_post' => array(
				'id' => $row['id_last_msg'],
				'member' => array(

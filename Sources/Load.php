...
function loadUserSettings()
{
	...
	$user_info += array(
		...
		'gender' => empty($user_settings['gender']) ? '' : $user_settings['gender'],
		'avatar' => array(
...
function loadBoard()
{
	...
	if (empty($temp))
	{
		$request = $smcFunc['db_query']('', '
			SELECT
				c.id_cat, b.name AS bname, b.description, b.num_topics, b.member_groups, b.require_prefix,
				b.id_parent, c.name AS cname, IFNULL(mem.id_member, 0) AS id_moderator, b.disabled_signatures,
		...
		if ($smcFunc['db_num_rows']($request) > 0)
		{
			...
			$board_info = array(
				...
				'disabled_signatures' => !empty($row['disabled_signatures']),
...
function loadMemberData($users, $is_name = false, $set = 'normal')
{
	...
	elseif ($set == 'minimal')
	{
		$select_columns = '
			...
			mem.posts, mem.last_login, mem.member_ip, mem.member_ip2, mem.lngfile, mem.id_group, mem.is_activated';
		$select_tables = '';
	}
	else
	...
	if (!empty($loaded_ids) && $set !== 'minimal')
	{
		...
		foreach ($user_profile as $key => &$value) {
			...
			if (!empty($value['additional_groups_ori'])) {
				$value['additional_groups_list'] = explode(',', $value['additional_groups_ori']);
				//Just load the complete additional membergroups details if enabled..
				if (empty($modSettings['show_additional_groups']))
					continue;
				$temp = array();
...
function loadMemberContext($user, $display_custom_fields = false)
{
	...
	//Deal with additional membergroups
	...
	$memberContext[$user] = array(
		...
		'is_guest' => false,
		'additional_group_stars' => $profile['post_group'],
...
function loadTheme($id_theme = 0, $initialize = true)
{
	...
	<script type="text/javascript" src="' . $settings['default_theme_url'] . '/scripts/jquery.jgrowl.min.js"></script>
	<link rel="stylesheet" type="text/css" href="' . $settings['default_theme_url'] . '/css/jquery.jgrowl.css" />
	<script language="JavaScript" type="text/javascript"><!--
	jQuery.noConflict();
	jQuery(document).ready(function($){
	  $.jGrowl.defaults.position = \''.$modSettings['imiyfposition'].'\';

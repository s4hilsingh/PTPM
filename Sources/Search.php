...
function PlushSearch1()
{
	...
	// Prefixes
	if (!empty($context['search_params']['prefix']))
		$context['search_params']['prefix'] = (int) $context['search_params']['prefix'];

	$context['search_params']['show_complete'] = !empty($context['search_params']['show_complete']);
	$context['search_params']['subject_only'] = !empty($context['search_params']['subject_only']);

	// Prefix?
	PostPrefix::getPrefix(0,true);
...
function PlushSearch2()
{
	...
	if (!empty($_REQUEST['prefix']))
		$search_params['prefix'] = (int) $_REQUEST['prefix'];
			...
			else
			{
				...
				if (empty($search_params['topic']))
				{
					...
					foreach ($searchWords as $orIndex => $words)
					{
						...
						{
							$subject_query['where'][] = 't.id_prefix = {int:prefix}';
							$subject_query['params']['prefix'] = $search_params['prefix'];
						}
						if (!empty($minMsgID))
						{
						...
						if (!empty($words['indexed_words']))
						{
							...
							$search_data = array(
								...
								'params' => array(
									...
									// Prefix
									'prefix' => !empty($search_params['prefix']) ? $search_params['prefix'] : 0,
				...
				else
				{
					...
					if (!empty($search_params['prefix']))
					{
						$main_query['where'][] = 't.id_prefix = {int:prefix}';
						$main_query['parameters']['prefix'] = $search_params['prefix'];
					}
					if (!empty($minMsgID))
					{
	...
	if (!empty($context['topics']))
	{
		...
		$messages_request = $smcFunc['db_query']('', '
			SELECT
				...
				b.id_board, b.name AS board_name, c.id_cat, first_mem.is_activated AS first_activated, last_mem.is_activated AS last_activated, c.name AS cat_name
...
function prepareSearchContext($reset = false)
{
	...
	if($message['last_activated'] > 10)
		$message['last_member_name'] = '<s>' . $message['last_member_name'] . '</s>';

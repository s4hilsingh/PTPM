...
function summary($memID)
{
	...
	$context += array(
		'page_title' => sprintf($txt['profile_of_username'], str_replace(array('<s>', '</s>'), array('', ''), $memberContext[$memID]['name'])),
...
function showPosts($memID)
{
	...
	// Set the page title
	$context['page_title'] = $txt['showPosts'] . ' - ' . str_replace(array('<s>', '</s>'), array('', ''), $user_profile[$memID]['real_name']);
...
function statPanel($memID)
{
	...
	$context['page_title'] = $txt['statPanel_showStats'] . ' ' . str_replace(array('<s>', '</s>'), array('', ''), $user_profile[$memID]['real_name']);
...
function tracking($memID)
{
	...
	// Set a page title.
	$context['page_title'] = $txt['trackUser'] . ' - ' . $subActions[$context['tracking_area']][1] . ' - ' . str_replace(array('<s>', '</s>'), array('', ''), $user_profile[$memID]['real_name']);

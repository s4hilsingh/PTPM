...
function Post()
{
	...
	if (!empty($topic))
	{
		$request = $smcFunc['db_query']('', '
			SELECT
				...
				t.id_first_msg, mf.subject'. (isset($_REQUEST['msg']) ? ', cm.id_member, cm.signature_id' : '') . ',
	...
	if (isset($_REQUEST['message']) || !empty($context['post_error']))
	{
		...
		if (isset($_REQUEST['msg']) && !empty($topic))
		{
			...
			$request = $smcFunc['db_query']('', '
				SELECT
					m.id_member, m.modified_time, m.smileys_enabled, m.body, t.id_prefix,
	...
	// Get the prefixes
	PostPrefix::getPrefix($context['current_board']);
...
function Post2()
{
	...
	$msgOptions = array(
		...
		'signature_id' => (isset($_POST['multiplesignatures']) ? advsig_getSignatureID($_POST['multiplesignatures'], isset($row['id_member']) ? $row['id_member'] : $user_info['id']) : (isset($_POST['stored_signature']) ? $_POST['stored_signature'] : 0)),
...
function getTopic()
{
	...
	$request = $smcFunc['db_query']('', '
		SELECT
			...
			m.body, m.smileys_enabled, m.id_msg, mem.is_activated, m.id_member
		FROM {db_prefix}messages AS m
			LEFT JOIN {db_prefix}members AS mem ON (mem.id_member = m.id_member)
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		$context['previous_posts'][] = array(
			'counter' => $counter++,
			'alternate' => $counter % 2,
			'poster' => $row['is_activated'] > 10 ? '<s>' . $row['poster_name'] . '</s>' : $row['poster_name'],
			'message' => $row['body'],
	...
	if (isset($_REQUEST['xml']))
	{
		...
		else
		{
			$context['message'] = array(
				...
				'error_in_id_prefix' => in_array('no_prefix', $post_errors),

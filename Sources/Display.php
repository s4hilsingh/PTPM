...
function Display()
{
	...
	$request = $smcFunc['db_query']('', '
		SELECT
			...
			t.id_member_started, t.id_first_msg, t.id_last_msg, t.approved, t.unapproved_posts, disabled_signatures,
	...
	$context['prefix_subject'] = PostPrefix::formatPrefix($topicinfo['id_prefix']). ' '.$topicinfo['subject'];
	...
	if ($context['is_poll'])
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT
				...
				p.guest_vote, p.id_member, IFNULL(mem.real_name, p.poster_name) AS poster_name, IFNULL(mem.is_activated, 0) AS is_activated, p.num_guest_voters, p.reset_poll
		...
		if($pollinfo['is_activated'] > 10)
			$pollinfo['poster_name'] = '<s>' . $pollinfo['poster_name'] . '</s>';
		$smcFunc['db_free_result']($request);
	...
	if (!empty($messages))
	{
		...
		if (!empty($modSettings['attachmentEnable']) && allowedTo('view_attachments'))
		{
			$request = $smcFunc['db_query']('', '
				SELECT
					...
					a.mime_type, a.width, a.height' . (empty($modSettings['attachmentShowImages']) || empty($modSettings['attachmentThumbnails']) ? '' : ',
		...
		$messages_request = $smcFunc['db_query']('', '
			SELECT
				...
				smileys_enabled, poster_name, poster_email, approved, signature_id,
...
function prepareDisplayContext($reset = false)
{
	...
	else
	{
		...
		$memberContext[$message['id_member']]['signature'] = advsig_getSignatureByID($message['signature_id'],$message['id_member']);
...
function Download()
{
	...
	// Is this an supported audio or video file?
...
function loadAttachmentContext($id_msg)
{
	...
	if (isset($attachments[$id_msg]) && !empty($modSettings['attachmentEnable']))
	{
		...
		{
			$attachment['mime_type'] = !isset($attachment['mime_type']) ? '' : $attachment['mime_type'];
			$attachmentData[$i] = array(
				...
				'mime_type' => !empty($attachment['mime_type']) ? $attachment['mime_type'] : false,
			);

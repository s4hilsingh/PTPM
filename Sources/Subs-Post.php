...
function sendpm($recipients, $subject, $message, $store_outbox = false, $from = null, $pm_head = 0)
{
	...
	$smcFunc['db_insert']('',
		...
		array(
			...
			'from_name' => 'string-255', 'msgtime' => 'int', 'subject' => 'string-255', 'body' => 'string-65534', 'signature_id' => 'int',
		),
		array(
			$pm_head, $from['id'], ($store_outbox ? 0 : 1),
			$from['username'], time(), $htmlsubject, $htmlmessage, $context['signature_id'],
		),
		array('id_pm')
	);
...
function createPost(&$msgOptions, &$topicOptions, &$posterOptions)
{
	...
	$smcFunc['db_insert']('',
		...
		array(
			...
			'smileys_enabled' => 'int', 'modified_name' => 'string', 'icon' => 'string-16', 'approved' => 'int', 'signature_id' => 'int',
		),
		array(
			$topicOptions['board'], $topicOptions['id'], $posterOptions['id'], $msgOptions['subject'], $msgOptions['body'],
			$posterOptions['name'], $posterOptions['email'], time(), $posterOptions['ip'],
			$msgOptions['smileys_enabled'] ? 1 : 0, '', $msgOptions['icon'], $msgOptions['approved'], $msgOptions['signature_id'],
		),
		array('id_msg')
	);
	...
	if ($new_topic)
	{
		$smcFunc['db_insert']('',
			...
			array(
				...
				$topicOptions['id_prefix'] == null ? 0 : $topicOptions['id_prefix'],
			),
			array('id_topic')
		);
	...
	else
	{
		...
		$smcFunc['db_query']('', '
			...
			array(
				...
				'id_topic' => $topicOptions['id'],
				'id_prefix' => $topicOptions['id_prefix'],
			)
		);
...
function createAttachment(&$attachmentOptions)
{
	...
	// Check to see if it is a audio or video file:
...
function modifyPost(&$msgOptions, &$topicOptions, &$posterOptions)
{
	...
	$messageInts = array('modified_time', 'id_msg_modified', 'smileys_enabled', 'signature_id');
	...
	if ($topicOptions['sticky_mode'] !== null || $topicOptions['lock_mode'] !== null || $topicOptions['poll'] !== null || $topicOptions['id_prefix'] != null)
	{
		$smcFunc['db_query']('', '
			UPDATE {db_prefix}topics
			SET
...
function loadEmailTemplate($template, $replacements = array(), $lang = '', $loadLang = true)
{
	...
	$ret = array(
		'subject' => isset($context['email_templates']['id_template'][$template]) && !empty($context['email_templates']['id_template'][$template]['subject']) ? un_htmlspecialchars($context['email_templates']['id_template'][$template]['subject']) : $txt['emails'][$template]['subject'],
		'body' => isset($context['email_templates']['id_template'][$template]) && !empty($context['email_templates']['id_template'][$template]['body']) ? un_htmlspecialchars($context['email_templates']['id_template'][$template]['body']) : $txt['emails'][$template]['body'],
	);

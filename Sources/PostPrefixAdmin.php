...
class PostPrefixAdmin
{
	...
	public static function general()
	{
		...
		$context['PostPrefix']['credits'] = PostPrefix::dev_credits();
	...
	public static function prefixes()
	{
		...
		if (isset($_REQUEST['added']) && !isset($_REQUEST['updated']) && !isset($_REQUEST['deleted']))
		{
			...
			$context['prefix']['addup'] .= '<div id="profile_success">'.sprintf(PostPrefix::text('prefix_added'), PostPrefix::formatPrefix($id)).'</div>';
		}
		elseif (isset($_REQUEST['updated']) && !isset($_REQUEST['added']) && !isset($_REQUEST['deleted']))
		{
			$id = (int) $_REQUEST['updated'];
			$context['prefix']['addup'] .= '<div id="profile_success">'.sprintf(PostPrefix::text('prefix_updated'), PostPrefix::formatPrefix($id)).'</div>';
		}
		elseif (isset($_REQUEST['deleted']) && !isset($_REQUEST['added']) && !isset($_REQUEST['updated']))
		{
			$context['prefix']['addup'] .= '<div id="profile_success">'.PostPrefix::text('prefix_deleted').'</div>';
		}
		...
		$listOptions = array(
			...
			'columns' => array(
				'status' => array(
					...
					'data' => array(
						'function' => create_function('$row', 'global $scripturl, $settings; return ($row[\'status\'] == 1 ? \'<a href="\'. $scripturl. \'?action=admin;area=postprefix;sa=ups;id=\'. $row[\'id\']. \';status=0"><img src="\'. $settings[\'images_url\'].\'/warning_watch.gif" style="vertical-align: middle;" alt="" /></a>\' : \'<a href="\'. $scripturl. \'?action=admin;area=postprefix;sa=ups;id=\'. $row[\'id\']. \';status=1"><img src="\'. $settings[\'images_url\'].\'/warning_mute.gif" style="vertical-align: middle;" alt="" /></a>\');'),
						'style' => 'width: 2%',
						'class' => 'centertext',
					),
					'sort' => array(
				...
				'id' => array(
					'header' => array(
						'value' => PostPrefix::text('prefix_id'),
						'class' => 'centertext',
					),
					'data' => array(
						'db' => 'id',
				...
				'boards' => array(
					...
					'data' => array(
						'sprintf' => array(
							'format' => '<a href="'. $scripturl. '?action=admin;area=postprefix;sa=showboards;id=%1$d" onclick="return reqWin(this.href);">'. PostPrefix::text('select_visible_boards'). '</a>',
							'params' => array(
								'id' => true,
								'name' => true,
							),
				...
				'groups' => array(
					...
					'data' => array(
						'sprintf' => array(
							'format' => '<a href="'. $scripturl. '?action=admin;area=postprefix;sa=showgroups;id=%1$d" onclick="return reqWin(this.href);">'. PostPrefix::text('select_visible_groups'). '</a>',
							'params' => array(
								'id' => true,
								'name' => true,
							),
	...
	public static function settings($return_config = false)
	{
		...
		// Set all the page stuff
		$context['page_title'] = PostPrefix::text('main') . ' - '. PostPrefix::text('tab_settings');
		$context[$context['admin_menu_name']]['tab_data'] = array(
			'title' => $context['page_title'],
			'description' => PostPrefix::text('tab_settings_desc'),
		);
		...
		$config_vars = array(
			array('check', 'PostPrefix_enable_filter', 'subtext' => PostPrefix::text('enable_filter_desc')),
			array('select', 'PostPrefix_select_order', array(
					PostPrefix::text('prefix_name'),
					PostPrefix::text('prefix_id'),
					PostPrefix::text('prefix_date'),
				),
				'subtext' => PostPrefix::text('select_order_desc')
			),
			array('select', 'PostPrefix_select_order_dir', array(
					PostPrefix::text('DESC'),
					PostPrefix::text('ASC'),
				),
				'subtext' => PostPrefix::text('select_order_dir_desc')
			),
		);

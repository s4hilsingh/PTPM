...
function MessageIndex()
{
	...
	if (isset($_REQUEST['sort']) && isset($_REQUEST['prefix']))
		$context['page_index'] = constructPageIndex($scripturl . '?board=' . $board . '.%1$d;sort=' . $_REQUEST['sort'] . (isset($_REQUEST['desc']) ? ';desc' : '').(isset($_REQUEST['prefix']) ? ';prefix='.$_REQUEST['prefix'] : ''), $_REQUEST['start'], $board_info['total_topics'], $maxindex, true);
	elseif (isset($_REQUEST['prefix']))
		$context['page_index'] = constructPageIndex($scripturl . '?board=' . $board . '.%1$d;'.(isset($_REQUEST['prefix']) ? ';prefix='.$_REQUEST['prefix'] : ''), $_REQUEST['start'], $board_info['total_topics'], $maxindex, true);
	...
	if (!empty($settings['display_who_viewing']))
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT
				lo.id_member, lo.log_time, mem.real_name, mem.member_name, mem.show_online, mem.is_activated,
	...
	if (!$pre_query || !empty($topic_ids))
	{
		...
		$result = $smcFunc['db_query']('substring', '
			SELECT
				t.id_topic, t.num_replies, t.locked, t.num_views, t.is_sticky, t.id_poll, t.id_previous_board, t.id_prefix,
				...
				IFNULL(meml.real_name, ml.poster_name) AS last_display_name, IFNULL(meml.is_activated, 0) AS last_mem_activated, t.id_first_msg,
				mf.poster_time AS first_poster_time, mf.subject AS first_subject, mf.icon AS first_icon,
				mf.poster_name AS first_member_name, mf.id_member AS first_id_member,
				IFNULL(memf.real_name, mf.poster_name) AS first_display_name, IFNULL(memf.is_activated, 0) AS first_mem_activated, SUBSTRING(ml.body, 1, 385) AS last_body,
			...
			WHERE ' . ($pre_query ? 't.id_topic IN ({array_int:topic_list})' : 't.id_board = {int:current_board}') . (!$modSettings['postmod_active'] || $context['can_approve_posts'] ? '' : '
				AND (t.approved = {int:is_approved}' . ($user_info['is_guest'] ? '' : ' OR t.id_member_started = {int:current_member}') . ')') . (isset($_REQUEST['prefix']) ? '
				AND t.id_prefix = {int:topic_prefix}' : ''). '
			...
			array(
				...
				'topic_prefix' => (isset($_REQUEST['prefix']) ? $_REQUEST['prefix'] : ''),
				'start' => $start,
				'maxindex' => $maxindex,
			)
		);

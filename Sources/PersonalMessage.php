...
function MessageFolder()
{
	...
	if (!empty($pms))
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT pmr.id_pm, mem_to.id_member AS id_member_to, mem_to.is_activated, mem_to.real_name AS to_name, pmr.bcc, pmr.labels, pmr.is_read
		...
		while ($row = $smcFunc['db_fetch_assoc']($request))
		{
			if ($context['folder'] == 'sent' || empty($row['bcc']))
				$recipients[$row['id_pm']][empty($row['bcc']) ? 'to' : 'bcc'][] = empty($row['id_member_to']) ? $txt['guest_title'] : '<a href="' . $scripturl . '?action=profile;u=' . $row['id_member_to'] . '">' . ($row['is_activated'] > 10 ? '<s>' . $row['to_name'] . '</s>' : $row['to_name']) . '</a>';
		...
		if ($context['display_mode'] != 0)
		{
			...
			$subjects_request = $smcFunc['db_query']('', '
				SELECT pm.id_pm, pm.subject, pm.id_member_from, pm.msgtime, IFNULL(mem.real_name, pm.from_name) AS from_name, mem.is_activated,
		...
		$messages_request = $smcFunc['db_query']('', '
			SELECT pm.id_pm,' . ($context['sort_by'] == 'name' ? ' mem.is_activated,' : '') . ' pm.subject, pm.id_member_from, pm.body, pm.msgtime, pm.from_name, pm.signature_id
...
function MessageSearch2()
{
	...
	if (!empty($foundMessages))
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT
				pmr.id_pm, mem_to.id_member AS id_member_to, mem_to.real_name AS to_name, mem_to.is_activated,
...
function MessagePost()
{
	...
	if (!empty($_REQUEST['pmsg']))
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT
				pm.id_pm, IFNULL(mem.is_activated, 0) AS is_activated, CASE WHEN pm.id_pm_head = {int:id_pm_head_empty} THEN pm.id_pm ELSE pm.id_pm_head END AS pm_head,
		...
		$context['quoted_message'] = array(
			...
			'member' => array(
				'name' => ($row_quoted['is_activated'] > 10 ? '<s>' . $row_quoted['real_name'] . '</s>' : $row_quoted['real_name']),
				'username' => $row_quoted['member_name'],
				'id' => $row_quoted['id_member'],
				'href' => !empty($row_quoted['id_member']) ? $scripturl . '?action=profile;u=' . $row_quoted['id_member'] : '',
				'link' => !empty($row_quoted['id_member']) ? '<a href="' . $scripturl . '?action=profile;u=' . $row_quoted['id_member'] . '">' . ($row_quoted['is_activated'] > 10 ? '<s>' . $row_quoted['real_name'] . '</s>' : $row_quoted['real_name']) . '</a>' : $row_quoted['real_name'],
			),
	...
	if (isset($_REQUEST['u']))
	{
		...
		if ($_REQUEST['u'] == 'all' && isset($row_quoted))
		{
			...
			$request = $smcFunc['db_query']('', '
				SELECT mem.id_member, mem.real_name, mem.is_activated
				FROM {db_prefix}pm_recipients AS pmr
			...
			while ($row = $smcFunc['db_fetch_assoc']($request))
				$context['recipients']['to'][] = array(
					'id' => $row['id_member'],
					'name' => ($row['is_activated'] > 10 ? '<s>' . $row['to_name'] . '</s>' : $row['to_name']),
				);
			$smcFunc['db_free_result']($request);
		}
		else
		{
			...
			$request = $smcFunc['db_query']('', '
				SELECT id_member, is_activated, real_name
				FROM {db_prefix}members
			...
			while ($row = $smcFunc['db_fetch_assoc']($request))
				$context['recipients']['to'][] = array(
					'id' => $row['id_member'],
					'name' => ($row['is_activated'] > 10 ? '<s>' . $row['real_name'] . '</s>' : $row['real_name']),
				);
			$smcFunc['db_free_result']($request);
		}
...
function messagePostError($error_types, $named_recipients, $recipient_ids = array())
{
	...
	if ($context['reply'])
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT
				pm.id_pm, IFNULL(mem.is_activated, 0) AS is_activated, CASE WHEN pm.id_pm_head = {int:no_id_pm_head} THEN pm.id_pm ELSE pm.id_pm_head END AS pm_head,
				pm.body, pm.subject, pm.msgtime, mem.member_name, IFNULL(mem.id_member, 0) AS id_member,
				IFNULL(mem.real_name, pm.from_name) AS real_name
			FROM {db_prefix}personal_messages AS pm' . ($context['folder'] == 'sent' ? '' : '
...
function MessagePost2()
{
	...
	$context['signature_id'] = (isset($_POST['multiplesignatures']) ? advsig_getSignatureID($_POST['multiplesignatures']) : $_POST['stored_signature']);
...
function ReportMessage()
{
	...
	else
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT pm.subject, pm.body, pm.msgtime, pm.id_member_from, IFNULL(m.real_name, pm.from_name) AS sender_name, m.is_activated
			FROM {db_prefix}personal_messages AS pm
				INNER JOIN {db_prefix}pm_recipients AS pmr ON (pmr.id_pm = pm.id_pm)
		...
		$request = $smcFunc['db_query']('', '
			SELECT mem_to.id_member AS id_member_to, mem_to.real_name AS to_name, pmr.bcc, mem_to.is_activated
			FROM {db_prefix}pm_recipients AS pmr
				LEFT JOIN {db_prefix}members AS mem_to ON (mem_to.id_member = pmr.id_member)
			WHERE pmr.id_pm = {int:id_pm}
				AND pmr.id_member != {int:current_member}',
		...
		while ($row = $smcFunc['db_fetch_assoc']($request))
		{
			...
			if ($row['bcc'])
				...
				$recipients[] = '[url=' . $scripturl . '?action=profile;u=' . $row['id_member_to'] . ']' . ($row['is_activated'] > 10 ? '<s>' . $row['to_name'] . '</s>' : $row['to_name']) . '[/url]';
		}
		$smcFunc['db_free_result']($request);

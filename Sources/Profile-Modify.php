...
function setupProfileContext($fields)
{
	...
	if(isset($context['profile_fields']['real_name']))
...
function forumProfile($memID)
{
	...
	setupProfileContext(
		array(
			...
			'icq', 'aim', 'msn', 'yim', 'hr',
			'usertitle', 'signature', 'random_signature', 'hr',
			'karma_good', 'hr',
...
function profileLoadGroups()
{
	...
	$curGroups = $cur_profile['additional_groups_list'];
...
function profileSaveGroups(&$value)
{
	...
	if (isset($_POST['additional_groups']) && is_array($_POST['additional_groups']))
	{
		...
		$old_additional_groups = $old_profile['additional_groups_list'];
		...
		if (implode(',', $additional_groups) !== implode(',', $old_profile['additional_groups_list']))
		{
			$profile_vars['additional_groups'] = implode(',', $additional_groups);
			$cur_profile['additional_groups_list'] = $additional_groups;
		}
	}

	// Too often, people remove delete their own account, or something.
	if (in_array(1, $old_profile['additional_groups_list']) || $old_profile['id_group'] == 1)
	{
...
function groupMembership($memID)
{
	...
	$groups = $curMember['additional_groups_list'];
...
function groupMembership2($profile_vars, $post_errors, $memID)
{
	...
	$addGroups = array_flip($old_profile['additional_groups_list']);
